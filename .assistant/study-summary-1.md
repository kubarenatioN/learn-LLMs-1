...
### Phase 4: Agents & Tools (5 lessons, COMPLETE)
...
- **Lesson 5:** Error handling & guardrails. `createAgent` catches tool exceptions by default (passes error text to LLM — LLM can recover but may hallucinate). `toolRetryMiddleware` — automatic retries with exponential backoff, transparent to LLM. `modelCallLimitMiddleware` — `runLimit` caps LLM calls per invocation (prevents infinite loops), `threadLimit` for cost budgets. `exitBehavior: 'end'` for graceful stop, `'error'` to throw. System prompt guardrails: explicit behavioral rules ("ONLY answer weather", "do NOT make up data") prevent hallucination and keep agent on-topic. Layers of defense: Zod schemas → tool error strings → middleware → system prompt → application try/catch.
